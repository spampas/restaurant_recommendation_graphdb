...
public final class RistogoGUI extends Application
{
	...
	private VBox buildPreferencesInterface()
	{
		...
		ChoiceBox<CityInfo> cityChoice = new ChoiceBox<CityInfo>();
	...
	/*private void getOwnRestaurant()
	{
		if (restaurantForm == null)
			return;
		ResponseMessage resMsg = Protocol.getInstance().listOwnRestaurants();
		if (!resMsg.isSuccess()) {
		...
		restaurant = (RestaurantInfo)resMsg.getEntity();
